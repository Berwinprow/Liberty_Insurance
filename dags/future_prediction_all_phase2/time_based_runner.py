"""
Time-based model evaluation runner for Future Prediction Phase-2.

Supports:
- Single models
- Ensemble models
- Seven-set undersampling logic
...
def compute_metrics(y_true, y_prob, threshold):
    """
    Compute classification metrics using a fixed threshold.
    """
...
def run_time_based(
    ...
    """
    STRICT time-based evaluation runner.

    Notes:
    - Threshold MUST be scalar (handled by orchestrator)
    - Supports single models, ensembles, and seven-set logic
    """
    ...
    logger.info(
        "TIME-BASED RUN STARTED | Model=%s | Sampling=%s | Threshold=%.3f",
        model_name,
        sampling_method,
        threshold,
    )
    ...
    # ======================================================
        ...
        if model_cfg["type"] == "single":
            ...
            if model_cfg["method"] == "soft_voting":
                ...
                model = build_weighted_ensemble(model_cfg, X_train, X_train, y_train)
    ...
    row = {
        ...
        "test_months": ",".join(map(str, time_cfg["validation"]["months"])),
    ...
    logger.info(
        "TIME-BASED RUN COMPLETED | Model=%s | Train=%d | Test=%d",
        model_name,
        len(X_train),
        len(X_test),
    )
