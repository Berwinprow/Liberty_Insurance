...
# HELPERS
# ======================================================
def parse_split(split: str) -> float:
    """Convert '80_20' → 0.2"""
...
def run_grid_param_queue(
    ...
    logger.info(
        "GRID START → %s | Params=%s | Parallel=%s",
        model_name,
        len(param_list),
        max_parallel,
    )
    ...
    logger.info("GRID DONE → %s", model_name)
...
def run_phase4_pipeline():
    """
    Execute Phase-4 grid search pipeline (ML + DL).
    """
    logger.info("PHASE-4 | GRID SEARCH (ML + DL) STARTED")

    base_df = load_and_clean_data()

    with open(GRID_CFG, "r") as file:
        grid_cfg = json.load(file)
    ...
    with open(ENCODE_CFG, "r") as file:
        encode_rules = json.load(file)

    target_col = conn_cfg["columns"]["target_column"]

    hook = PostgresHook(
        postgres_conn_id=conn_cfg["connection"]["postgres_conn_id"]
    )
    ...
    for fs_name, fs_cfg in grid_cfg.items():
        ...
        for split_name, split_cfg in fs_cfg["splits"].items():
            ...
            for sampling, rule in split_cfg["sampling"].items():
                ...
                logger.info("DL GRID SEARCH STARTED")
