"""
Ensemble training and evaluation utilities.
"""
...
def run_ensemble(
    ...
    """
    Train, evaluate, and persist ensemble model results.
    ...
    Args:
        ...
        y_test: Test target
        meta: Metadata dictionary (feature_set, split, sampling)
        cfg: Ensemble configuration dictionary
    """
    # ================= LOAD CONFIG =================
    ...
    if already_trained(
        ...
        logger.info(
            "SKIPPED ENSEMBLE | %s (already trained)",
            ensemble_name,
        )
        return

    logger.info("STARTED ENSEMBLE | %s", ensemble_name)
    ...
    logger.info("ENSEMBLE COMPLETED | %s", ensemble_name)
